...
config:
	@[ -f $(CONFIG_FILE) ] && printf "WARNING: $(CONFIG_FILE) file already exists \nIf you really wish to replace the config.toml file then please delete it first" || (echo "Copying default config to $(CONFIG_FILE)" && cp ./config/config.example.toml config.toml && sed -i '1,4{2{/DO NOT EDIT THIS FILE/!b};d}' config.toml)
